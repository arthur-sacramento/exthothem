...
if($lastDotIndex){
    ...
    $saveFolder = "categories/$ext/$currentDate";

    // Create the folder if it doesn't exist
    if (!is_dir($saveFolder)) {
      mkdir($saveFolder, 0755, true);

      $fp = fopen("categories/$ext/pages.html", "a");
      fwrite($fp, "<a href='files.php?search=$ext/$currentDate'>$currentDate</a><br>");
      fclose($fp);
    }
    ...
    foreach ($parts as $part) {
        $folderName = $categoriesDir . '/' . $part . '/' . $currentDate;
        ...
        if (!is_dir($folderName)) {
           ...
           $fp = fopen("$categoriesDir/$part/pages.html", "w");
           fwrite($fp, "<a href='files.php?search=$part/$currentDate'>$currentDate</a><br>");
           fclose($fp); 

        }

        if ($ext === "img") {
            $fileContent = "<a href='../../img/$currentDate/$filenameFinal'><img src='../../img/$currentDate/$filenameFinal'></a>";
        } else {
